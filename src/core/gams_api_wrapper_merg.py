...
# Legacy compatibility wrapper that matches original gams_api_wrapper.py interface
class GamsApiWrapper:
    """
    Legacy compatibility wrapper that provides the same interface as the original GamsApiWrapper.
    
    This maintains backward compatibility while using the superior v49 implementation underneath.
    """
...
class GamsTransferWrapper:
    ...
    Note: For new code, use gdx_io_merg.py functions which properly separate
    Control API and Transfer API usage.
    """
